...
export default function ProductDetailPage() {
  ...
  const handleReviewSubmit = async (e) => {
    ...
    const already = reviews.find(r => r.user?._id === user?._id || r.user === user?._id)
    if (already) return toast.error('You have already reviewed this product')

    setReviewSubmitting(true)
    try {
      // ✅ Use product._id for API call (backend needs ObjectId)
      const { data } = await productsAPI.addReview(product._id, { rating: reviewRating, comment: reviewText.trim() })

      // Use the real saved review from DB (has real _id + createdAt)
      ...
      // Update product rating with values returned from backend
      setProduct(prev => ({
        ...prev,
        numReviews: data.numReviews ?? (prev.numReviews || 0) + 1,
      ...
      // ✅ Re-fetch product to get fresh reviews from DB (bypasses any cache)
      try {
        const { data: fresh } = await productsAPI.getOne(product.slug || product._id, true) // true = bypass cache
        if (fresh?.product) {
      ...
      <div className="bg-cream min-h-screen">
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 pb-16">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16">
            ...
            <div>
              {/* Main image */}
              <div className="relative bg-gradient-to-br from-stone to-cream aspect-square flex items-center justify-center overflow-hidden group">
            ...
            <div>
              <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
                ...
                <div className="flex items-center gap-1">
                  <span className="text-gold text-sm">{'★'.repeat(Math.round(product.rating || 5))}{'☆'.repeat(5 - Math.round(product.rating || 5))}</span>
                  <span className="text-warm/60 text-xs">({product.numReviews || 0} reviews)</span>
                </div>
              </div>
              ...
              <p className="text-warm/70 text-sm leading-relaxed mb-6">
              ...
              {product.craftingDays && (
                <div className="mt-4 bg-stone/40 border border-stone px-4 py-3 text-sm text-warm/70">
            ...
            <div className="flex border-b border-stone overflow-x-auto">
              {TABS.map(tab => (
                <button key={tab} onClick={() => setActiveTab(tab)}
                  ...
                  {tab}{tab === 'Reviews' ? ` (${product.numReviews || reviews.length})` : ''}
                </button>
              ))}
            </div>
                ...
                <div>
                  <div className="flex flex-col md:flex-row gap-10 mb-10">
                    <div className="text-center md:w-48 flex-shrink-0">
                      <p className="font-serif text-6xl text-gold">{(product.rating || 0).toFixed(1)}</p>
                      <div className="flex justify-center text-gold text-xl my-2">
                        {'★'.repeat(Math.round(product.rating || 0))}{'☆'.repeat(5 - Math.round(product.rating || 0))}
                      </div>
                      <p className="text-warm/50 text-sm">{product.numReviews || 0} reviews</p>
                    </div>
                      ...
                      <form onSubmit={handleReviewSubmit} className="space-y-4">
                        ...
                        {!isAuthenticated && (
                          <p className="text-xs text-warm/50 mt-1">
                            <Link href="/auth/login" className="text-gold underline">Login</Link> to submit a review
                          </p>
                        )}
                      </form>
                    </div>
                  ...
                  <div className="space-y-4">
                    {reviews.length === 0 && (
                      <p className="text-center text-warm/40 text-sm py-6">No reviews yet. Be the first to review!</p>
                    )}
                    {reviews.map(r => (
                      <div key={r._id || r.id} className="bg-white border border-stone p-5">
                        <div className="flex items-start justify-between mb-3">
                          <div>
                            <div className="flex items-center gap-2">
                              <p className="font-medium text-charcoal text-sm">
                                {r.name || `${r.user?.firstName || ''} ${r.user?.lastName || ''}`.trim() || 'Customer'}
                              </p>
                            </div>
                            <div className="flex text-gold text-sm mt-0.5">{'★'.repeat(Math.max(1, Math.min(5, r.rating || 5)))}</div>
                          </div>
                          <div className="text-right">
                            ...
                            {r.isVerifiedPurchase && <p className="text-[10px] text-green-600 mt-0.5">✓ Verified Purchase</p>}
                          </div>
                        </div>
